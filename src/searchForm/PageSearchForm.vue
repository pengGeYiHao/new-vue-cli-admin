  ...
  <div class="pageSearchForm">
    <el-collapse accordion @change="collapseHandle" v-model="activeNames">
      <el-collapse-item name="true">
        <template slot="title">
          <div style="display: flex;justify-content: space-between;padding-right:10px;background-color: #fff;">
        ...
        <el-form :model="form" size="small" style="margin-top: 25px;">
          <div style="display: flex;flex-wrap: wrap;justify-content: space-between;margin-right: 25px;">
...
export default {
  ...
  methods: {
    ...
    searchOnChange() {
      ...
      if (startT != "" && endT != "") {
        this.getListParams.startTime = this.formatTimeYYMMDD(
          new Date(startT).toLocaleDateString().split("/")
        );
        this.getListParams.endTime = this.formatTimeYYMMDD(
          new Date(endT).toLocaleDateString().split("/")
        );
      } else {
      ...
      //当无分页时this.fetchData(this.getListParams)
    },
